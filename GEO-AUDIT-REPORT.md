...
su copia de repositorio), por lo que la ficha declaraba 100 URLs antes de esta ronda; las tres fuentes nuevas de los artículos P0 dejaron 103 y las dos fuentes de campo añadidas a coral dejan ahora 105 URLs. Los estados `203/403` se
...
- `pnpm content:source-render-audit`: las 32 fichas construidas exponen las 105 URLs de fuente del frontmatter como enlaces visibles en el HTML; 0 rutas de artículo ausentes y 0 fuentes omitidas. El control quedó integrado en `.github/workflows/content-quality.yml`.
- `pnpm content:source-metadata-audit`: las 105 fuentes del archivo tienen editor, URL, tipo de evidencia y alcance declarado (105/105), y 70/105 incluyen `accessedDate`. La auditoría falla si una ficha con `reviewedDate` y `reviewedBy` deja una fuente sin fecha de acceso; las 35 fechas pendientes se completan durante la revisión humana. El control quedó integrado en `.github/workflows/content-quality.yml` para evitar que una nueva ficha pierda contexto editorial o trazabilidad.
...
- `pnpm content:reference-audit`: las 105 fuentes del frontmatter también aparecen como enlaces exactos en el cuerpo de sus artículos (105/105); el control evita bibliografías genéricas o fuentes registradas que el lector no pueda rastrear.
