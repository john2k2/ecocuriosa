...
**Audit date:** 20 de septiembre de 2026
**Última actualización documental:** 20 de septiembre de 2026 (investigación, métricas, revisión visual y planificación; no cambia las métricas privadas ni la revisión de AdSense)
...
La segunda evaluación independiente de Jev está archivada en
[`docs/editorial/JEV_REVIEW_2026-09-20.md`](docs/editorial/JEV_REVIEW_2026-09-20.md).
Es un juicio ordinal auxiliar, no una métrica de Google, Cloudflare o AdSense;
su prioridad coincide con la deuda demostrable de revisión humana E‑E‑A‑T.
...
**Refresco autenticado de la misma consulta — 14 de septiembre de 2026 (segunda lectura).** Al volver a consultar la ventana 8–14/09, el total observado pasó a 13.025 solicitudes y 181.466.516 bytes; 2.083 solicitudes y 55.204.565 bytes fueron servidos desde caché (16,0 % y 30,4 %). El 14/09 seguía siendo un día en curso al momento de la lectura, por lo que el corte no se trata como cierre diario ni como crecimiento de audiencia. La tabla conserva el detalle y la advertencia de alcance.
...
**Reenvío autenticado — 14 de septiembre de 2026.** Tras publicar `/correcciones/`, se volvió a enviar `https://ecocuriosa.com/sitemap-index.xml`. Search Console lo muestra enviado y leído el 14/09, con estado **Correcto** y 44 URL descubiertas. El XML público contiene 44 URL; el conteo de descubiertas ya coincide, pero no se declara que las 44 estén indexadas hasta observarlo explícitamente en cobertura.

**Refresco de rendimiento autenticado — 14 de septiembre de 2026.** La
ventana nominal de tres meses muestra 340 impresiones, 2 clics, CTR 0,6 % y
posición media 12; el gráfico visible cubre del 5 al 12/09. Las consultas con
mayor exposición son `architeuthis dux` (26), `pulpo mimo` (15) y `geodinamo`
(14). Por dispositivo, el ordenador concentra 210 impresiones y el móvil
129; por país, España registra 70 y México 65. El corte completo, incluidas
...
su copia de repositorio), por lo que la ficha declaraba 100 URLs antes de esta ronda; las tres fuentes nuevas de los artículos P0 dejaron 103, las dos fuentes de campo añadidas a coral dejaron 105 y las dos fuentes primarias de reproducción del calamar dejan ahora 107 URLs. Los estados `203/403` se
...
| Medición reproducible posterior (una ejecución, 14/09/2026) | 93/100 | 1,7 s | 2,8 s | 0 | 140 ms | Móvil; accesibilidad 100/100, SEO 100/100 y Best Practices 81/100; oportunidades sintéticas en imágenes, CSS render-blocking y scripts gestionados |
| Seguimiento después de `srcset` responsive (una ejecución, 14/09/2026) | 99/100 | 1,0 s | 1,7 s | 0 | 100 ms | Móvil; descargó WebP de 800 px en tarjetas; la auditoría de entrega de imágenes quedó sin bytes desperdiciados; Best Practices 81/100 por scripts gestionados |
| Después de precargar Albert Sans (una ejecución, 20/09/2026) | 93/100 | 1,4 s | 2,7 s | 0 | 130 ms | Móvil; accesibilidad 100/100 y SEO 100/100; render-blocking estimado bajó a 500 ms; sigue siendo laboratorio |
...
- `pnpm content:source-render-audit`: las 32 fichas construidas exponen las 107 URLs de fuente del frontmatter como enlaces visibles en el HTML; 0 rutas de artículo ausentes y 0 fuentes omitidas. El control quedó integrado en `.github/workflows/content-quality.yml`.
...
- `pnpm content:source-metadata-audit`: las 107 fuentes del archivo tienen editor, URL, tipo de evidencia y alcance declarado (107/107), y 74/107 incluyen `accessedDate`. La auditoría falla si una ficha con `reviewedDate` y `reviewedBy` deja una fuente sin fecha de acceso; las 33 fechas pendientes se completan durante la revisión humana. El control quedó integrado en `.github/workflows/content-quality.yml` para evitar que una nueva ficha pierda contexto editorial o trazabilidad.
- `pnpm content:review-precheck -- --write`: la cola de 31 pendientes muestra ahora el número de fechas de acceso por artículo; 17/31 fichas tienen cobertura completa y el resto recibe una acción explícita para completarla durante la revisión humana. El prechequeo no marca fichas como revisadas.
- `pnpm content:reference-audit`: las 107 fuentes del frontmatter también aparecen como enlaces exactos en el cuerpo de sus artículos (107/107); el control evita bibliografías genéricas o fuentes registradas que el lector no pueda rastrear.
...
- `pnpm content:generated-metadata-audit`: recorre 47 archivos HTML generados, identifica 44 páginas indexables y valida idioma, viewport, títulos, descripciones, canonicales, Open Graph, JSON-LD y duplicados; comprueba además la estructura de `FAQPage` y evita exponerla en fichas pendientes. El control quedó integrado en `.github/workflows/content-quality.yml`.
- `pnpm content:indexation-audit`: comprueba los 32 artículos publicados y las 12 páginas estáticas indexables, su HTML y canonical, su aparición única en el sitemap (44/44), `Allow: /`, el sitemap canónico y el `noindex` de búsqueda/404; no detecta problemas. El control quedó integrado en `.github/workflows/content-quality.yml`.
- `pnpm content:navigation-audit`: comprueba en 46 páginas HTML la navegación principal y móvil, el estado inicial del menú, la relación `aria-controls`/`id`, la sincronización del estado y el foco al abrir; no detecta incidencias ni IDs duplicados. El control quedó integrado en `.github/workflows/content-quality.yml`.
...
- `SOURCE_CATALOG.yml`: YAML válido con **592 entradas** y cobertura de las 107 URLs citadas por los artículos; la biblioteca mantiene 130 oportunidades editoriales. La ronda del 20/09 añadió trece candidatas oficiales para imágenes responsive/LCP, Lighthouse, accesibilidad, redirects, meta tags, JSON-LD, ads.txt, contenido asistido por IA, transparencia editorial, experiencias de IA en Search e identidad pública verificable, más doce fuentes nuevas de Luna para autoridad temática, validación de DOI/PMID, correcciones, autoría autenticada, metadatos DataCite y calidad/reproducibilidad de GBIF. Las tandas anteriores de Luna y la revisión dirigida del 14/09 añadieron candidatas para SEO, medición, móvil, accesibilidad, AdSense, autoría y actualización científica, siempre como insumos de investigación y no como citas automáticas. Todas permanecen candidatas hasta la comprobación editorial de la fuente concreta. El detalle está en el [snapshot de acceso a fuentes](docs/editorial/SOURCE_ACCESS_SNAPSHOT_2026-09-12.md) y en la sección de fuentes abiertas de la [biblioteca de inspiración](docs/editorial/EDITORIAL_INSPIRATION_LIBRARY.md).
- Conteo actualizado tras la ronda más reciente: `SOURCE_CATALOG.yml` contiene **592 entradas**. Se añadieron doce controles nuevos para HTML renderizado, inventario de crawl, CMP/TCF, cobertura de mensajes, separación laboratorio/campo, RUM, interacción táctil, ORCID y estados bibliográficos de PubMed, tres controles de Crossmark, metadatos Crossref y relaciones de corrección en PubMed, y ocho controles nuevos de aporte original, ORCID autenticado, roles CRediT, guías EQUATOR, principios FAIR, versiones DataCite, política editorial MEDLINE e integridad de referencias PubMed; siguen siendo candidatas hasta la comprobación editorial.
